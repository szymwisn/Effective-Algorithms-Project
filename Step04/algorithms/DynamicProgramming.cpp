...
int DynamicProgramming::execute(int city, int mask) {
    ...
    for (int nextCity = 0; nextCity < this->numberOfCities; nextCity++) {
        // continue if city hasn't been visited yet
        if (!hasTheCityBeenVisited(nextCity, mask)) {
...
bool DynamicProgramming::hasTheCityBeenVisited(int city, int mask) {
...
void DynamicProgramming::collectPaths() {
    ...
    for(int i = 0; i < numberOfCities; i++) {
        ...
        // if the last city was already found - stop collecting citiesList
        if (city == -1)
            break;
...
void DynamicProgramming::allocateMemory() {
    // this->citiesList possible citiesList that could have been visited as the last ones and
    // 2^this->citiesList (1 << this -> citiesList) possible subsets of visited citiesList
